...
# Briefer 서버 URL
BRIEFER_URL = "http://localhost:8005/extracted_file/"
...
def process_files_and_notify(file_ids: List[int]):
    ...
    if completed_ids:
        try:
            ...
            resp = requests.post(BRIEFER_URL, json=payload, timeout=5)
            print(f"✅ Briefer notified for {len(completed_ids)} files. Status: {resp.status_code}")
        except Exception as e:
            print(f"⚠️ Briefer notification failed: {e}")
